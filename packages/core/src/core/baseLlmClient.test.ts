...
describe('BaseLlmClient', () => {
  ...
  describe('AbortSignal integration', () => {
    beforeEach(() => {
      // Reset the retry mock to actually pass through the abort signal for these tests
    ...
    it('should pass abortSignal to retryWithBackoff', async () => {
      const mockResponse = createMockResponse('{"color": "blue"}');
      mockGenerateContent.mockResolvedValue(mockResponse);
    ...
    it('should handle aborted signal correctly', async () => {
      ...
      await expect(client.generateJson(options)).rejects.toThrow('Aborted');
    ...
    it('should not report errors when aborted', async () => {
      const controller = new AbortController();

      // Setup retry mock to simulate abort during retry
      ...
      // Should not report abort as an application error
      expect(reportError).not.toHaveBeenCalled();
    });

    it('should handle abort signal correctly in retry integration', async () => {
      const controller = new AbortController();
      controller.abort();
    ...
    it('should work without abortSignal (backward compatibility)', async () => {
      ...
      const optionsWithoutSignal: GenerateJsonOptions = {
        contents: [{ role: 'user', parts: [{ text: 'Give me a color.' }] }],
        schema: { type: 'object', properties: { color: { type: 'string' } } },
        model: 'test-model',
        promptId: 'test-prompt-id',
      ...
      const result = await client.generateJson(optionsWithoutSignal);
